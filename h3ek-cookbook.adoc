...
[source, CMD]
----
tool scenario-merge levels\(your_level_type)<4>\(your_level_name)\(your_level_name)<5>
----
<4> The level type is the 'folder name' that bungie used to organize level types in the levels folder. The Known Level Types are: Solo, Multi, Ui. also known as Campaign, Multiplayer, Mainmenu.

<5> This is the folder that contains the scenario tag you want to merge.
