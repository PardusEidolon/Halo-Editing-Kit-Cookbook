...
[source, CMD]
----
tool scenario-merge levels\(your_level_type)\(your_level_folder)\(your_level_name) <1>
----
<1> The level type is the 'folder name', Bungie used this to organize level types in the levels folder. The Known Level Types are: `Solo`, `Multi`, `UI`. also known as `campaign`, `multiplayer` and `main-menu`.
